...
const Intervals = () => {
  ...
  const handleGenerateIntervals = () => {
    ...
    data.forEach((entry) => {
      if (entry[_TYPE] === "Scheduled") {
        if (
          !scheduledAuxs.includes(entry[_ACTIVITY]) &&
        ...
        if (
          !actualAuxs.includes(entry[_ACTIVITY]) &&
          entry[_ACTIVITY] !== "ACTIVITY"
        ) {
      ...
      for (let i = 0; i < intervals.length; i++) {
        if (intervals[i] >= considerPM(entry[_FROM], true)) {
          if (considerPM(entry[_TO]) >= intervals[i] + selected.interval) {
            ...
            if (entry[_TYPE] === "Scheduled") {
              output.scheduled[entry[_ACTIVITY]][i] +=
                considerPM(entry[_TO]) - intervals[i]
            } else {
              output.actual[entry[_ACTIVITY]][i] +=
                considerPM(entry[_TO]) - intervals[i]
            }
          }
        } else if (
          intervals[i] + selected.interval >
          considerPM(entry[_FROM], true)
        ) {
          if (considerPM(entry[_TO]) >= intervals[i] + selected.interval) {
            if (entry[_TYPE] === "Scheduled") {
              output.scheduled[entry[_ACTIVITY]][i] +=
            ...
            if (entry[_TYPE] === "Scheduled") {
              output.scheduled[entry[_ACTIVITY]][i] +=
                considerPM(entry[_TO]) - considerPM(entry[_FROM], true)
            } else {
              output.actual[entry[_ACTIVITY]][i] +=
                considerPM(entry[_TO]) - considerPM(entry[_FROM], true)
            }
    ...
    for (let i = 0; i < intervals.length; i++) {
      ...
      totalsRow = totalsRow.map((val, index) => {
        if (val !== "Totals") {
          return val + newRow[index]
        } else {
          return val
        }
      })
    ...
    setPercentage({
      actual: {
        isConverted: true,
        data: { header: actualPercent[0], entries: actualPercent.slice(1) },
      },
      scheduled: {
        isConverted: true,
        data: {
          header: scheduledPercent[0],
          entries: scheduledPercent.slice(1),
        },
      },
    })
  ...
  const handleSelectedBars = (aux) => {
    if (selected.bars.includes(aux)) {
      setSelected({
        ...selected,
        bars: selected.bars.filter((val) => val !== aux),
      })
    } else {
  ...
  const handleSelectedAreas = (aux) => {
    if (selected.areas.includes(aux)) {
      setSelected({
        ...selected,
        areas: selected.areas.filter((val) => val !== aux),
      })
    } else {
  ...
  const handleExportChartData = (headers) => {
    ...
    let data = chartData.map((chartItem) =>
      headerRow.map((aux) => chartItem[aux])
    )
  ...
  const handleSelectActOpen = (aux) => {
    if (selected.actOpen.includes(aux)) {
      setSelected({
        ...selected,
        actOpen: selected.actOpen.filter((val) => val !== aux),
      })
    } else {
  ...
  const handleSelectSchOpen = (aux) => {
    if (selected.schOpen.includes(aux)) {
      setSelected({
        ...selected,
        schOpen: selected.schOpen.filter((val) => val !== aux),
      })
    } else {
    ...
    <Fragment>
      ...
      <main className="mb-4 container">
        ...
        <div className=" d-flex flex-column align-items-center text-center my-4">
          <h3>UPLOADS</h3>
          <CSVUploader
            loadedHandler={handleUploadIntervals}
            removeHandler={() => setLoaded({ ...loaded, intervals: false })}
            header="Intervals CSV"
            label="Insert Intervals CSV"
          />
        </div>
        <div className="container d-flex flex-column align-items-center text-center my-2">
          ...
          <div className="d-flex align-items-center my-2">
            <button
              className="btn btn-outline-dark btn-sm my-2 mx-2"
              onClick={handleGenerateIntervals}
              disabled={entries.type !== "intervals"}
            >
              Generate Intervals
            </button>
            <Dropdown className="mx-1">
              ...
              >
                <Dropdown.Item
                  onClick={() => setSelected({ ...selected, date: null })}
                >
                  All Dates
                </Dropdown.Item>
                {entries.dates &&
            ...
            <Dropdown className="mx-1">
              ...
              >
                <Dropdown.Item
                  onClick={() => setSelected({ ...selected, agent: null })}
                >
                  All Agents
                </Dropdown.Item>
                {entries.agents &&
                  entries.agents.map((agent) => (
                    <Dropdown.Item
                      onClick={() => setSelected({ ...selected, agent: agent })}
                    >
                      {agent}
                    </Dropdown.Item>
                  ))}
            ...
            <Dropdown className="mx-1">
              ...
              <Dropdown.Menu>
                <Dropdown.Item
                  onClick={() => setSelected({ ...selected, interval: 15 })}
                >
                  15'
                </Dropdown.Item>
                <Dropdown.Item
                  onClick={() => setSelected({ ...selected, interval: 30 })}
                >
                  30'
                </Dropdown.Item>
                <Dropdown.Item
                  onClick={() => setSelected({ ...selected, interval: 60 })}
                >
                  60'
                </Dropdown.Item>
              </Dropdown.Menu>
          ...
          {generated.intervals && (
            <div className="d-flex justify-content-center border p-2 m-2 shadow-sm">
              <input
                type="text"
                placeholder="Custom File Name"
                value={exportsCustomName}
                onChange={(e) => setExportsCustomName(e.target.value)}
              ></input>
              <CSVDownloader
                data={exports}
          ...
          {chartData && (
            <div className="mt-2">
              ...
              <div className="container">
                {intervals &&
                  intervals.scheduledAuxs.map((aux) => (
                    <button
                      key={"select-bar-" + aux}
                      className={
                        selected.bars.includes("SCH_" + aux)
                          ? "btn btn-sm btn-danger m-1"
                          : "btn btn-sm btn-outline-primary m-1"
                      }
                      onClick={() => handleSelectedBars("SCH_" + aux)}
                    >
                      {"SCH_" + aux}
                    </button>
                  ))}
                <br />
                {intervals &&
                  intervals.actualAuxs.map((aux) => (
                    <button
                      key={"select-bar" + aux}
                      className={
                        selected.bars.includes(aux)
                          ? "btn btn-sm btn-danger m-1"
                          : "btn btn-sm btn-outline-secondary m-1"
                      }
                      onClick={() => handleSelectedBars(aux)}
                    >
                      {aux}
                    </button>
                  ))}
                ...
                {intervals &&
                  intervals.scheduledAuxs.map((aux) => (
                    <button
                      key={"select-area-" + aux}
                      className={
                        selected.areas.includes("SCH_" + aux)
                          ? "btn btn-sm btn-danger m-1"
                          : "btn btn-sm btn-outline-primary m-1"
                      }
                      onClick={() => handleSelectedAreas("SCH_" + aux)}
                    >
                      {"SCH_" + aux}
                    </button>
                  ))}
                ...
                {intervals &&
                  intervals.actualAuxs.map((aux) => (
                    <button
                      key={"select-area" + aux}
                      className={
                        selected.areas.includes(aux)
                          ? "btn btn-sm btn-danger m-1"
                          : "btn btn-sm btn-outline-secondary m-1"
                      }
                      onClick={() => handleSelectedAreas(aux)}
                    >
                      {aux}
                    </button>
                  ))}
              ...
              <button
                ...
                onClick={() =>
                  setSelected({ ...selected, bars: [], areas: [] })
                }
              >
                Reset Chart
              </button>
              <CSVDownloader
                data={handleExportChartData([
                  ...selected.areas,
                  ...selected.bars,
                ])}
                filename={`intervals_${
                  selected.date
                    ...
                    : entries.dates[0] +
                      "_to_" +
                      entries.dates[entries.dates.length - 1]
                }`}
                config={{ encoding: "ISO-8859-1" }}
              >
                <button className="btn btn-success btn-sm mx-2">
          ...
          {generated.intervals && (
            <div>
              ...
              <div className="container mb-3">
                {intervals.scheduledAuxs &&
                  intervals.scheduledAuxs.map((header) => (
                    <button
                      key={"select-sch-" + header}
                      className={
                        selected.schOpen.includes(header)
                          ? "btn btn-sm btn-danger m-1"
                          : "btn btn-sm btn-outline-primary m-1"
                      }
                      onClick={() => handleSelectSchOpen(header)}
                    >
                      {header}
                    </button>
                  ))}
              ...
              <div className="container">
                {intervals.actualAuxs &&
                  intervals.actualAuxs.map((header) => (
                    <button
                      key={"select-act-" + header}
                      className={
                        selected.actOpen.includes(header)
                          ? "btn btn-sm btn-danger m-1"
                          : "btn btn-sm btn-outline-primary m-1"
                      }
                      onClick={() => handleSelectActOpen(header)}
                    >
                      {header}
                    </button>
                  ))}
              ...
              <button
                className="btn btn-secondary btn-sm"
                onClick={() =>
                  setSelected({ ...selected, actOpen: [], schOpen: [] })
                }
              >
                Reset Chart
              </button>
              ...
              <button
                ...
                onClick={() => {
                  navigator.clipboard.writeText(
                    `${chartData.map((row) => row["SCH OPEN"]).join("\n")}`
                  )
                }}
              >
                Copy SCH
              ...
              <button
                ...
                onClick={() => {
                  navigator.clipboard.writeText(
                    `${chartData.map((row) => row["ACT OPEN"]).join("\n")}`
                  )
                }}
              >
                Copy ACT
